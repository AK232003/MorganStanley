  ...
  const toggle = () => setDropdownOpen(!dropdownOpen);
	...
	<div className="container mt-4 overflow-y-scroll bg-color2">
        ...
        <DropdownMenu className="text-textcolor">
          <DropdownItem onClick={()=>setFilter(t('Name'))}>{t('Name')}</DropdownItem>
          <DropdownItem onClick={()=>setFilter(t('District'))}>{t('District')}</DropdownItem>
          <DropdownItem onClick={()=>setFilter(t('Case Number'))}>{t('Case Number')}</DropdownItem>
        </DropdownMenu>
      </Dropdown>
			</div>
		</div>
