...
const ChildrenProfile = ({user}) => {
  ...
  const toggleModal = (caseno) =>{
    ...
    const childrenLists=()=>{
        return (
            ...
            {children.filter(children => {
				...
				<button className="m-2 p-2 rounded-pill bg-color4 text-textcolor w-full" onClick={()=>toggleModal(children["Case Number"])}> Profile</button>
				</div>
				<CardBody>
								<List type="unstyled">
	...
	<div className="container lg:mt-4 overflow-y-scroll bg-color2">
		...
		{/* Assign Worker */}
		<Modal centered isOpen={modal} toggle={toggleModal} fullscreen="md" size="lg">
