...
// ----------------------------------------
  ...
  const toggleModal = (caseno) =>{
    ...
    const childrenLists=()=>{
        return (
            ...
            {children.filter(children => {
				...
				<button className="m-2 p-2 rounded-pill bg-color4 text-textcolor w-full" onClick={()=>toggleModalDeadline(children["id"])}> Set Deadline</button>
					</CardBody>
				</Card>
            )})}
        </div>)
	...
	<div className="container lg:mt-4 overflow-y-scroll bg-color2">
        ...
        <ModalHeader toggle={toggleModal}>Comments for tasks related to {caseSelected}</ModalHeader>
        <ModalBody>
			...
			{/* Case Details */}
		<Modal centered isOpen={modalDeadline} toggle={toggleModalDeadline} fullscreen="md" size="sm">
        <ModalHeader toggle={toggleModalDeadline}>Deadlines for {caseSelected}</ModalHeader>
