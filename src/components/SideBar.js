...
const SideBar=({setuser})=>{
	...
	return (
	<div className={`h-9/10 p-4 ${openSide ? "w-72" : "w-24"} bg-themecolor duration-300 rounded-3 m-2 relative shadow-xl opacity-90 hover:shadow-themecolor hover:opacity-100`}>
          ...
          <ul className="pt-2 ps-0">
            ...
            <li className="text-xl text-gray-300 bg-themecolor rounded-md p-2 flex items-center gap-x-4 mt-2 hover:bg-themecolor/[0.8]">
              <span><FaArrowAltCircleRight className="text-3xl text-textcolor block float-left cursor-pointer" onClick={()=>setuser(null)} ></FaArrowAltCircleRight></span>
