  ...
  useEffect(() => {
	...
	return(
		...
		<div className={`flex flex-row w-full h-screen`}>
			...
			<div className={`grid grid-rows-10 flex-col w-full ${open && "blur-sm"}`}>
				<NavBar className="row-start-1" user={user} open={open} setOpen={(value)=>setOpen(value)} toggle={toggle}/>
				<div className={`row-span-9 container mx-auto mt-0 lg:ms-2 bg-color2 overflow-y-scroll max-h-screen}`}>
					<Outlet/>
				</div>
			</div>
		</div>
